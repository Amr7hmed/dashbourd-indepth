...
export default function Employees() {
  ...
  const options = {
    method: "get",
    url: `${process.env.REACT_APP_API_BASEURL}/api/admin/employees?page=${currentPage}`,
    headers: {
...

    {
      ...
      render: (text, record) => (
        <Space size="middle" key={record.id}>
          <Link to={`/EditEmployee/${record.id}`}>Edit</Link>
          <DeleteBtn
            info={record}
            user={user}
            url={"/api/admin/employees/"}
            setData={setData}
            msg="You Deleted the employee"
            msg2="Are you sure delete this employee?"
